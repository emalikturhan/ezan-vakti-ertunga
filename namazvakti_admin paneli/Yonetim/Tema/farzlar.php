...
if($control == 1){
$cek = $db->prepare("SELECT * FROM Farzlar ORDER BY id DESC LIMIT 1");
$cek->execute();
foreach ($cek as $key) {
$data = $key["data"];
}
...
$up = $db->prepare("UPDATE Farzlar SET data=?");
...
$warning .= '</div>';
echo "<meta http-equiv='refresh' content='2;url=index.php?paginal=farzlar'>";
}
}
...
<h2 class="page-header-title">54 Farz</h2>
<div>
</div>
</div>
</div>
