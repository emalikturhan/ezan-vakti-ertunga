...
$baslik = "";
$icerik = "";
$cek = $db->prepare("SELECT * FROM Hadisler WHERE id =:id");
...
$ekle = $db->prepare("INSERT INTO Hadisler SET baslik=?,data=?");
...
$warning .= '</div>';
echo "<meta http-equiv='refresh' content='2;url=index.php?paginal=hadisler'>";
}
}
}
...
$guncelle = $db->prepare("UPDATE Hadisler SET baslik=?,data=? WHERE id=?");
...
<a href="index.php?paginal=hadisler" class="btn btn-gradient-03">Hadisler Listesi</a>
</div>
</div>
</div>
</div>
...
if($anahtar == 1){
?>
<h4>Hadis Güncelle</h4>
<?php
} else {
?>
<h4>Yeni Hadis Ekle</h4>
<?php
}
?>
</div>
...
<label class="col-lg-2 form-control-label">Hadis Adı</label>
<div class="col-lg-10">
...
<div class="form-group row d-flex align-items-center mb-5">
<label class="col-lg-12 form-control-label">Hadis İçeriği</label>
<div class="col-lg-12">
