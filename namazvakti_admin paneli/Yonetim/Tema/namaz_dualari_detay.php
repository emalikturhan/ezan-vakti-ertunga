...
$baslik = "";
$icerik = "";
$cek = $db->prepare("SELECT * FROM NamazDualari WHERE id =:id");
...
$ekle = $db->prepare("INSERT INTO NamazDualari SET baslik=?,data=?");
...
$warning .= '</div>';
echo "<meta http-equiv='refresh' content='2;url=index.php?paginal=namaz_dualari'>";
}
}
}
...
$guncelle = $db->prepare("UPDATE NamazDualari SET baslik=?,data=? WHERE id=?");
...
<h2 class="page-header-title">Namaz Duaları</h2>
<div>
<div class="page-header-tools">
<a href="index.php?paginal=namaz_dualari" class="btn btn-gradient-03">Namaz Duaları Listesi</a>
</div>
</div>
</div>
</div>
...
if($anahtar == 1){
?>
<h4>Namaz Duası Güncelle</h4>
<?php
} else {
?>
<h4>Yeni Namaz Duası Ekle</h4>
<?php
}
?>
</div>
...
<div class="form-group row d-flex align-items-center mb-5">
<label class="col-lg-12 form-control-label">Namaz Duası İçeriği</label>
<div class="col-lg-12">
<textarea name="data" id="sayfa_icerik" rows="10" cols="80"  class="ckeditor"><?php echo @$icerik; ?></textarea>
</div>
</div>
